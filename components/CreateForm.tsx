...
const CreateForm: React.FC<CreateFormProps> = ({ onSaveSet, initialSet, onCancel }) => {
  ...
  const generateAIQuiz = async () => {
    if (!knowledge.trim() && images.length === 0) return alert('入力してください');
    setIsGenerating(true);
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const prompt = aiMode === 'create' 
        ? `新しく ${problemCount} 問作成してください。難易度:${difficulty}%` 
        : `デジタル化してください。`;

      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: { parts: [{ text: `${prompt} 日本語、選択肢5つ、JSON。` }] },
        config: {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10 pb-48">
      ...
      <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border border-white/5">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-8">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-black text-2xl tracking-tight">AI ASSISTANT</h3>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          <div>
            ...
            <select value={problemCount} onChange={(e) => setProblemCount(Number(e.target.value))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none">
              {[1, 3, 5, 10].map(n => <option key={n} value={n} className="text-slate-900">{n}問</option>)}
            </select>
          </div>
          <div>
            <label className="text-[10px] font-black opacity-40 uppercase mb-2 block tracking-widest text-center">Difficulty</label>
            <input type="range" min="0" max="100" step="10" value={difficulty} onChange={(e) => setDifficulty(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer accent-white mt-4" />
          </div>
          <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            ...
            className="flex-1 bg-white text-slate-900 py-5 rounded-[1.5rem] font-black flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50"
          >
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
        ...
        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="例: 国家試験直前まとめ" className="w-full px-6 py-5 rounded-3xl bg-slate-50 border-none font-black text-xl text-slate-800 placeholder-slate-300 focus:ring-4 focus:ring-indigo-500/10" required />
      </div>

      {/* Manual Problems List */}
      <div className="space-y-10">
        {problems.map((p, pIdx) => (
          <div key={p.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl relative animate-in slide-in-from-bottom-5 duration-500">
            <button type="button" onClick={() => setProblems(prev => prev.filter((_, i) => i !== pIdx))} className="absolute top-8 right-8 text-slate-200 hover:text-red-500"><Trash2 className="w-7 h-7" /></button>
            <div className="flex items-center gap-4 mb-8">
              <span className="bg-indigo-600 text-white text-xs font-black px-4 py-1.5 rounded-full tracking-tighter">QUESTION {pIdx + 1}</span>
            ...
            <textarea value={p.question} onChange={(e) => updateProblem(pIdx, { question: e.target.value })} className="w-full px-6 py-5 rounded-[2rem] bg-slate-50 border-none text-[18px] font-bold text-slate-800 min-h-[120px] mb-8" placeholder="問題文を入力..." />

            <div className="space-y-4 mb-10">
              <label className="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">Choices & Answer</label>
              {p.choices?.map((c, cIdx) => (
                <div key={c.id} className="flex gap-4">
                  ...
                  }} className={`w-12 h-12 rounded-full shrink-0 flex items-center justify-center transition-all ${p.correctIndices?.includes(cIdx) ? 'bg-green-500 text-white shadow-lg' : 'bg-slate-100 text-slate-300'}`}>
                  ...
                  }} className="flex-1 px-6 py-4 rounded-2xl bg-slate-50 border-none font-medium" />
                </div>
              ))}
            </div>
              ...
              <div className="space-y-3">
                {p.comments?.map((com, cIdx) => (
                  <div key={cIdx} className="flex gap-2">
                    ...
                    <button type="button" onClick={() => updateProblem(pIdx, { comments: p.comments?.filter((_, i) => i !== cIdx) })} className="text-red-300"><X className="w-5 h-5" /></button>
                  </div>
                ))}
                <button type="button" onClick={() => updateProblem(pIdx, { comments: [...(p.comments || []), ''] })} className="w-full py-3 bg-white border-2 border-dashed border-indigo-200 rounded-2xl text-indigo-400 text-[10px] font-black">+ ADD NOTE</button>
              </div>
            </div>
          </div>
        ))}

        <button type="button" onClick={() => setProblems([...problems, { id: crypto.randomUUID(), question: '', choices: [{ id: '1', text: '' }, { id: '2', text: '' }], correctIndices: [0], isMultipleChoice: false, explanation: '', category: 'compulsory', comments: [] }])} className="w-full py-10 border-4 border-dashed border-slate-200 rounded-[3rem] text-slate-300 font-black hover:border-indigo-200 hover:text-indigo-400 transition-all">+ ADD NEW QUESTION</button>
      </div>

      {/* Global Action Footer */}
      <div className="fixed bottom-0 left-0 right-0 z-[100] flex justify-center p-6 pb-12 bg-white/60 backdrop-blur-xl border-t border-slate-100">
        <div className="w-full max-w-xl flex gap-4">
          {onCancel && <button type="button" onClick={onCancel} className="px-10 bg-slate-100 text-slate-500 rounded-[2rem] font-black text-lg shadow-sm">BACK</button>}
          <button type="submit" className="flex-1 bg-indigo-600 text-white py-6 rounded-[2rem] font-black text-xl shadow-2xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-[0.98]">
            SAVE ALL CHANGES
          </button>
        </div>
      </div>
